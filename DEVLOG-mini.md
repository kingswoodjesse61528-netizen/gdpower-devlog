...
- **起因**：查 gdpower 近三日 MAE 时发现 07-07 断档（归档漏跑），补齐后近三日均值抬到 61.6，
  且 07-06/07/08 rMAE 全 >1（连续跑输 naive「抄昨天」基线）。逐时段拆解发现误差**几乎全集中在傍晚→晚间 16–23 时**
  （19–22 晚高峰 rMAE 2.88），晚高峰预测被摁在 320–385，实际冲到 550/572/695，**峰值削 40–55%**。
- **两次重训候选均无用**：分别用 `--test-start 06-07`、`07-06` 重训暂存候选并回测。**在双方共同 OOS 窗口上都打不过现役生产**
  （新候选 07-06~08 均 MAE 27.75 vs 生产 27.04，略差）。且教师强制回测里 07-08 rMAE 仍 1.18 → 证明不是「模型旧了」，重训无解。两候选已删。
- **真正根因**（在 `api_server.run_prediction`）：明日 24 行一次性建出、`日前电价=NaN`，导致**头号特征 `price_lag1`（重要度~0.31）**
  除 00:00 外全 NaN，`.ffill()` 把它塌成**常数（昨日 23:00 价）贯穿全天** → 整条预测被摁平。这是压峰真凶，与金山/负荷输入无关。
  （顺带查明：金山 AirScript 只返回到当天、无次日行，`/defaults` 接次日负荷这条走不通也非主因。）
- **修复（naive 版 Fix A）**：用「昨日同小时的前一小时价」(= `price_lag24` 右移一格，预测时即可得的真实价) 作 `price_lag1` 代理，
  恢复日内形状；首行取历史最后真值。**30 天 A/B 验证**：全局 MAE **−10.4%**、近端 OOS(07-03+) **−16.1%**、
  **赢 naive 天数 11→23**（翻倍）；07-06/07/08 日前 MAE 53/70/62 → **51/46/43**。回退 10 天，集中在异常/行情突变日（昨日形状误导）。
- **状态**：✅ 补丁已打进 `api_server.py`（`df_pred` 切出后、ext_defaults 兜底前），经**真实 run_prediction 代码路径**复验数值一致。
  **仅改 serving/归档路径，不碰 backtest.py**。备份 `api_server.py.bak_lag1fix_20260708_161424`。
- **改动文件**：`api_server.py`；新增 `tools/backfill_predict_0707.py`（补算某天漏跑归档，改 `TARGET` 复用）；
  `predictions/prediction_2026-07-07.csv` + accuracy.csv 07-07 行（MAE 69.64）。
- **下一步**：① 补丁**次日 11:10 update.sh Step2 重启 API 后自动生效**（无需手动）；要即刻生效则 `bash install_service.sh restart`。
  ② 观察上线后 accuracy.csv 是否如 A/B 预期下降。③ 可选增强：对异常日（昨日波动异常大）回退到 flat 或加权，压掉最坏的几个回退日。
- **坑**：CLAUDE.md 写的重训「全量 refit 生产 pkl / `prod_full_refit=true`」**当前 `retrain_model.py` 并未实现**——落盘的是 train-only 模型
  （`fit(X_train)` 直接 dump）。判断候选实际训练范围要以此为准（staged 候选 pkl 只训练到 test-start 前一天，可能比现役还旧）。doc 与代码不符，待订正。
